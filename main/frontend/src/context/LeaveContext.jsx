...
// off — so a leave request from someone in HR (Admin/Ops) or IT routes to
// the Founder for approval instead of showing up in HR's own approve/reject
// queue. Everyone else's leave still goes through HR as normal.
export function isFounderApproval(request) {
  const employee = employees.find((e) => e.id === request.employeeId);
  return employee?.department === 'Admin/Ops' || employee?.department === 'IT';
}
