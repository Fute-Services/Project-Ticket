...
import { getAllITComplaints, updateITStatus } from '../utils/api';
import { IT_CATEGORIES, STATUSES, PRIORITIES } from '../utils/constants';
import { Search, Inbox } from 'lucide-react';

export default function ITDashboard() {
  const { complaints, setComplaints, loading } = useComplaints(getAllITComplaints);
  ...
  async function handleStatusChange(id, newStatus) {
    try {
      const { data } = await updateITStatus(id, newStatus);
  ...
  return (
    <div className="min-h-screen hero-bg">
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        <h1 className="text-2xl font-bold text-white mb-1">IT Dashboard</h1>
        <p className="text-sm text-white/40 mb-6">All complaints raised to the IT department</p>
